...
\chapter{Motivation}
I personally find it an interesting computer science challenge to extract ingredients from plain text and cooking in general is fun anyway. But that would be a bad justification for a master thesis. Therefore this chapter points out some of many applications, which are possible, once the ingredients are extracted. 
...
\parencite{Freyne2010} also build a \textbf{recipe recommendation} system based on ingredients. Its goal is to help with healthy diets.

\parencite{ingredientNetworks} analyse, how far a recipe is liked in relation with its nutrition information, which are based on the ingredient of the recipe. This allows conclusions about the \textbf{health awareness}. They also analyse how far an \textbf{ingredient can be replaced through another ingredient}, which is handy, if it is not available right now, or if you want a low-fat alternative.  
